...
from torch.fx.utils import lazy_format_graph_code

log = logging.getLogger(__name__)
graph_code_log = torch._logging.getArtifactLogger(__name__, "graph_code")


def get_example_value_key(node: fx.Node) -> Optional[str]:
    """
...
def insert_deferred_runtime_asserts(
    ...
    for node in graph.nodes:
        ...
        with graph.inserting_before(
            ...
            example_value_key = get_example_value_key(node)
            if not example_value_key:
                continue
            ...
            match_symbol(node.meta[example_value_key], lambda: node)
            if isinstance(t := node.meta[example_value_key], torch.Tensor):
